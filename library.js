...
function showEmptyTableState() {
    document.querySelector(".table__cell--empty").classList.remove("table__cell--hidden");
}
function removeEmptyTableState() {
    document.querySelector(".table__cell--empty").classList.add("table__cell--hidden");
}
...
function loadBooksFromStorage() {
    ...
    if (books === null || books.length === 0 || localStorage.getItem("storedBooks") === '[]') {
        books = [];
        showEmptyTableState();
    }
    else {
...
function onReadStatusBtn(bookId) {
    ...
    thisReadStatusBtn = document.querySelector(`tr[data-id="${bookId}"]`).querySelector(".btn__iconLabel");
...
function createRow(thisBook) {
    ...
    readStatusBtn.addEventListener("click", () => onReadStatusBtn(thisBook.bookId));
